...
export default function AddIncomePopup() {
  // useContext
  const { setAddIncomePopup, setIncomeData } = useContext(AppContext);
  ...
  const handleAddIncome = async (e) => {
    ...
    try {
      const response = await axiosInstance.post("/api/v1/income/add", {
        icon,
        source,
        amount,
        date,
  ...
  return (
    <section className="fixed top-0 left-0 bottom-0 right-0 bg-zinc-800/60 flex items-center justify-center px-3 py-3 z-50">
      <div className="w-full md:w-145 bg-white text-zinc-800 p-4 rounded relative">
        <h1 className="text-2xl font-medium mb-6">Add Income</h1>
        <IoClose
          className="absolute top-2 right-2 text-2xl cursor-pointer"
          onClick={() => setAddIncomePopup(false)}
        />
