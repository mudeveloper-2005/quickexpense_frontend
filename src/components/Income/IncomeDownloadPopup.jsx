...
export default function IncomeDownloadPopup() {
  // useContext
  ...
  const handleDownload = async (e) => {
    ...
    try {
      const response = await axiosInstance.get(
        `/api/v1/income/downloadexcel?from=${from}&to=${to}`,
        {
          responseType: "blob",
        },
      );
      ...
      link.href = url;
      link.setAttribute("download", `income_details ${from}-${to}.xlsx`);
      document.body.appendChild(link);
      link.click();
      link.parentNode.removeChild(link);
      window.URL.revokeObjectURL(url);
      toast.success("Downloaded successfully");
      setIncomeDownloadPopup(false);
    } catch (error) {
    ...
    <section className="fixed top-0 left-0 bottom-0 right-0 bg-zinc-800/60 flex items-center justify-center px-3 py-3 z-50">
      <div className="w-full md:w-120 bg-white text-zinc-800 p-4 rounded relative">
        ...
        <IoClose
          ...
          onClick={() => setIncomeDownloadPopup(false)}
        />
